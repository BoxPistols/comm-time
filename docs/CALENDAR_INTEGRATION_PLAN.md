...
#### FR-0: Google ログインの追加（Phase 1）【決定済み】

- FR-0.1 既存のメール + パスワード認証に加え、Supabase Auth の Google プロバイダによるログインを追加する
- FR-0.2 同一メールアドレスの既存アカウントとは Supabase の自動アカウントリンクで統合する（メール確認済みが条件）
- FR-0.3 Google ログインは**認証のみ**に使用し、カレンダーのトークン管理は FR-1 の独自 OAuth で行う（責務分離。3.2 参照）
...
- FR-2.0 **カレンダータブをアプリ起動時のデフォルトタブとする**（未連携ユーザーには連携導線を表示）【決定済み】
...
| 方式 | 評価 |
|---|---|
| A. Supabase Auth の Google ログインの provider_token を流用 | ❌ Supabase はリフレッシュトークンの永続管理を保証しない。ログイン時に毎回カレンダースコープの同意を求めることになる |
| **B. カレンダー連携は独自 OAuth（採用）** | ✅ スコープを最小化でき、連携解除も独立。トークンのライフサイクルを完全に制御できる |

> 【決定済み】ログイン手段としての Google ログイン（Supabase Auth プロバイダ、スコープは profile/email のみ）は**追加する**。ただしカレンダーのトークンは上記 B のとおり独自 OAuth で管理し、両者は分離する。Google でログイン済みのユーザーがカレンダー連携する際は、同じ Google アカウントが選択済みの状態になるため追加の負担はワンクリックの同意のみ。
...
- 既存 `tab-switcher.tsx`（meeting / pomodoro）に **calendar タブを追加**し、`TabType` を拡張。**calendar を起動時のデフォルトタブとする**（未連携時は連携導線を表示）
...
1. Google ログイン追加（Supabase プロバイダ設定 + `auth-dialog.tsx` にボタン追加 + アカウントリンク確認）（1d）
...
### 4.1 決定事項（2026-07-10 レビュー）

1. **カレンダーを起動時のデフォルトタブにする**（FR-2.0）。未連携ユーザーには連携導線を表示
2. **Google ログインを追加する**（FR-0）。メール + パスワード認証は維持し、Supabase Auth の Google プロバイダを併設。カレンダーのトークン管理（独自 OAuth）とは分離

### 4.2 未決事項（オーナーの判断が必要なもの）

各項目に推奨案を付す。指定がなければ推奨案で実装する。

| # | 決めること | 選択肢 | 推奨 |
|---|---|---|---|
| 1 | **デフォルトビュー**: カレンダータブを開いた時の初期表示 | 今日 / 週 / アジェンダ | **今日**（毎朝見る画面として最短で価値が出る） |
| 2 | **初期表示カレンダー**: 連携直後にどのカレンダーを表示するか | プライマリのみ / 全カレンダー | **プライマリのみ**（設定でいつでも追加可能。誕生日等のノイズを避ける） |
| 3 | **辞退済み予定の扱い** | 表示 / 非表示 / グレーアウト | **非表示**（設定でトグル可能に） |
| 4 | **週の開始曜日・時刻表記** | 日曜/月曜、12h/24h | **月曜・24h**（日本のビジネス利用想定） |
| 5 | **過去の予定の保持期間**: キャッシュ・注釈検索の遡り範囲 | 30日 / 90日 / 無制限 | **90日**（振り返り需要とストレージのバランス） |
| 6 | **書き込みスコープの取得タイミング**（FR-1.2） | 最初から events スコープ / Phase 3 で再同意 | **Phase 3 で再同意**（審査が軽く、心理的ハードルも低い） |
| 7 | **TODO→カレンダー反映（FR-5.2）の要否** | 実装する / 見送り | Phase 2 リリース後の利用状況を見て判断 |
| 8 | **通知チャネル**（Phase 4） | Web Push / アプリ内のみ | Phase 4 着手時に判断 |

### 4.3 オーナー側で必要な作業（開発と並行して依頼）

実装ではなくアカウント・運用面で、リポジトリ外の作業が必要になる。

1. **Google Cloud プロジェクトの作成と OAuth クライアント発行**（3.2 参照）。発行された Client ID / Secret を Vercel の環境変数に設定
2. **OAuth 同意画面の設定**: アプリ名・サポートメール・**プライバシーポリシー URL が必須**。comm-time にはプライバシーポリシーページが現状ないため、作成が必要（本文はこちらでドラフト可能）
3. **Supabase ダッシュボードで Google プロバイダを有効化**（FR-0 用。同じ OAuth クライアントを利用可）
4. **Google の sensitive scope 審査への申請**（一般公開する場合）。テストユーザー100人以内の運用なら不要
