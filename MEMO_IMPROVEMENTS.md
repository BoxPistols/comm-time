...
- **Ctrl+Esc** (または Cmd+Esc): 全画面表示を解除
- **Cmd+S** (または Ctrl+S): 保存（編集中のみ）

**アーキテクチャ**:

- グローバルキーボードイベントリスナーは`MemoSwiper`コンポーネントで一元管理
- `MarkdownMemo`はローカルハンドラ（Cmd+S保存）のみ処理
- アクティブなメモ（表示中のスライド）のみがショートカットに反応
- 複数のメモがレンダリングされても、リスナーは1つだけ
...
- 修飾キー（Ctrl/Cmd）を使用することで、通常の入力と競合しない
