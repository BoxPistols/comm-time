...
export function CommTimeComponent() {
    ...
    const [pomodoroStartTime, setPomodoroStartTime] = useState<Date | null>(
        null
    )
    ...
    useEffect(() => {
        ...
        setAlarmPoints(
            getStorageValue('alarmPoints', initialMeetingAlarmPoints)
        )
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            ...
            localStorage.setItem(
                'tickSoundVolume',
                JSON.stringify(tickSoundVolume)
            )
  ...
  */
        ...
        if (isMeetingRunning && meetingStartTime) {
            timer = setInterval(() => {
                ...
                if (countdownMode && targetEndTime) {
                    ...
                    const [hours, minutes] = targetEndTime
                        .split(':')
                        .map(Number)
        ...
        if (isPomodoroRunning && pomodoroStartTime) {
            timer = setInterval(() => {
                ...
                setPomodoroElapsedTime(
                    Math.floor(
                        (now.getTime() - pomodoroStartTime.getTime()) / 1000
                    )
                )
                // チクタク音を再生
                playTickSound()
    ...
    const restoreTodo = useCallback(
        (trashedTodo: TrashedTodoItem) => {
            ...
            // ゴミ箱から削除
            setTrashedTodos((prev) =>
                prev.filter((t) => t.id !== trashedTodo.id)
            )
        },
        [useDatabase, user, sharedSupabaseTodos]
    )
    ...
    const linkTodoToAlarmPoint = useCallback(
        (todoId: string, alarmPointId: string) => {
            setAlarmPoints((prev) =>
                prev.map((point) =>
                    point.id === alarmPointId
                        ? { ...point, linkedTodo: todoId }
                        : point
                )
            )
        },
                    ...
                    <div className='flex flex-col sm:flex-row items-center justify-between gap-4'>
                        ...
                        <div className='flex gap-2 items-center'>
                            ...
                            <button
                                type='button'
                                onClick={() =>
                                    setTickSoundEnabled(!tickSoundEnabled)
                                }
                                        ...
                                        <div className='bg-white dark:bg-gray-700 rounded-lg p-3 sm:p-4'>
                                            <label className='flex flex-col sm:flex-row sm:items-center gap-2'>
                                                ...
                                                <input
                                                    type='number'
                                                    value={
                                                        meetingAlarmSettings.frequency
                                                    }
                                                    onChange={(e) =>
                                                        setMeetingAlarmSettings(
                                                            {
                            ...
                            <div className='flex gap-2'>
                                <input
                                    ...
                                    onKeyDown={(e) => {
                                        ...
                                        if (e.key === 'Enter') {
                                            const value =
                                                todoInputRef.current?.value.trim() ||
                                                ''
                                            if (value) {
                                                addTodo(
                                                    value,
                                                        ...
                                                        <div className='grid grid-cols-2 gap-2'>
                                                            <label className='flex flex-col gap-1'>
                                                                <span className='text-xs text-gray-600 dark:text-gray-400'>
                                                                    音量:{' '}
                                                                    {
                                                                        defaultPomodoroWorkAlarm.volume
                                                                    }
                                                                </span>
                                                                <input
                                                                    type='range'
                                                            ...
                                                            <label className='flex flex-col gap-1'>
                                                                <span className='text-xs text-gray-600 dark:text-gray-400'>
                                                                    周波数:{' '}
                                                                    {
                                                                        defaultPomodoroWorkAlarm.frequency
                                                                    }
                                                                    Hz
                                                                </span>
                                                                <input
                                                        ...
                                                        <div className='grid grid-cols-2 gap-2'>
                                                            <label className='flex flex-col gap-1'>
                                                                <span className='text-xs text-gray-600 dark:text-gray-400'>
                                                                    音量:{' '}
                                                                    {
                                                                        defaultPomodoroBreakAlarm.volume
                                                                    }
                                                                </span>
                                                                <input
                                                                    type='range'
                                                            ...
                                                            <label className='flex flex-col gap-1'>
                                                                <span className='text-xs text-gray-600 dark:text-gray-400'>
                                                                    周波数:{' '}
                                                                    {
                                                                        defaultPomodoroBreakAlarm.frequency
                                                                    }
                                                                    Hz
                                                                </span>
                                                                <input
